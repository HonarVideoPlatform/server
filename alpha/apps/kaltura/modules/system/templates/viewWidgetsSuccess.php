...
function addRow($widget_log, $hshow_data, $entry_data, $even_row )
{
	...
	$contributors = $hshow_data ? $hshow_data->getContributors() : 0;
	$entries = $hshow_data ? $hshow_data->getEntries() : 0;
...
// fetch #contribs and #entries per hshow
$hshow_ids = array();
$entry_ids = array();
foreach($widget_log_list as $widget_log)
{
	$hshow_id = $widget_log->getHshowId();
	if ($hshow_id)
		$hshow_ids[] = $hshow_id;
...
$c = new Criteria();
$c->add(hshowPeer::ID, $hshow_ids, Criteria::IN);
$hshows = hshowPeer::doSelect($c);


$hshows_data = array();
foreach($hshows as $hshow)
	$hshows_data[$hshow->getId()] = $hshow;
...
foreach($widget_log_list as $widget_log)
{
	 $media_content .= addRow($widget_log, @$hshows_data[$widget_log->getHshowId()], @$entries_data[$widget_log->getEntryId()], ( $i % 2 == 0 ) );
	++$i;
}
				...
				<table cellspacing="0" cellpadding="0">
					<thead>
						<tr>
							...
							<td class="type" onclick='changeMediaSortOrder (this, "+hshow_id")'><span>HshowId</span></td>
