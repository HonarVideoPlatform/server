...
class getPartnerReportAction extends kalturaSystemAction
{
	public function execute()
	{
	    ...
	    $query = "SELECT ID, HSHOW_ID, NAME from entry where partner_id=$partner_id and type=$entry_type";
	    ...
	    while ($resultset->next())
	    {
	    	...
	    	$hshow_id = $resultset->getString('HSHOW_ID');
	    	$name = $resultset->getString('NAME');
	    	
	    	//echo "$entry_id $hshow_id $name<br/>\n";
        	$all_entries[$entry_id] = array("hshow_id" => $hshow_id, "name" => str_replace(',',' ',$name));
	    }
		...
		for($i = 0; $i < 2; $i++)
		{
			...
			{
		        $s = $entry_id.",".$all_entries[$entry_id]["hshow_id"].",".$all_entries[$entry_id]["name"].",";
