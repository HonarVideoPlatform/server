...
class myFeatureUtils
{
	 ...
	 * The ids of the featured hshows will be read from a config file at first, and at a later phase for DB.  
	 *
	 */
	public static function getFeaturedTeams ()
	{
		return self::getCachedHshowList ( "featured_team_list" );
	}
	...
	{
		return self::getCachedHshowList ( "featured_show_list" );
	}
	...
	{
		return self::getCachedHshowList ( "featured_intro_list" );
	}
	
	
	
	private  static function getCachedHshowList ( $config_param_name , $fallback = NULL) 
	{
		return array();
		self::init();
		...
		if ( $cached_show_list == NULL )
		{
			...
			$c = new Criteria();
			$c->add ( hshowPeer::ID , $id_list , Criteria::IN );
			// fetch the real hshows from the DB
			$hshow_list = hshowPeer::doSelectJoinkuser( $c );

			// create stub objects with only the minimum data from the real hshow
			...
			{
				$temp_array[$hshow->getId()] = new hshowStub ( $hshow ); 
			}
