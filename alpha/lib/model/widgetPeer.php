...
{
	const WIDGET_PEER_JOIN_HSHOW = 1;
...
*/
	...
	public static function retrieveByPK($pk, PropelPDO $con = null, $join = null)
	{
		...
		// for now supporting only hshow,entry,hshow+entry and all 
		if ( $join == self::WIDGET_PEER_JOIN_HSHOW )
			$res = self::doSelectJoinhshow( $c , $con );
		elseif ( $join == self::WIDGET_PEER_JOIN_ENTRY )
			$res = self::doSelectJoinentry( $c , $con );
		elseif ( $join == self::WIDGET_PEER_JOIN_ENTRY + self::WIDGET_PEER_JOIN_HSHOW )
