...
class DocumentsService extends KalturaEntryService
{
	...
	{
		$ksUnique = $this->getHsUniqueString();
	...
	public function serveByFlavorParamsIdAction($entryId, $flavorParamsId = null, $forceProxy = false)
	{
		...
		$referrer = null;
		if (isset($_GET["forceProxy"]) && hString::beginsWith($_GET["forceProxy"], "true/referrer/"))
		{
		...
		$hsObj = $this->getHs();
		$hs = ($hsObj) ? $hsObj->getOriginalString() : null;
		$securyEntryHelper = new KSecureEntryHelper($dbEntry, $hs, $referrer, ContextType::DOWNLOAD);
		$securyEntryHelper->validateForDownload();
